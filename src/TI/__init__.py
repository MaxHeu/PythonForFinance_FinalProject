...
class TechInd:
    ...
    def bollinger_band(self, df: pd.DataFrame, band_factor: float = 2.0) -> pd.DataFrame:
        df['upper_bb'] =  df[f'ma_{self.window_bb}'] + (band_factor * df[f'vol_{self.window_bb}'])
        df['lower_bb'] = df[f'ma_{self.window_bb}'] - (band_factor * df[f'vol_{self.window_bb}'])
        return df

    def rsi(self, df: pd.DataFrame) -> pd.DataFrame:
        df['rsi'] = ta.momentum.rsi(df[f'ma_{self.window_bb}'])
        return df
